...
async fn main() -> std::io::Result<()> {
    ...
    let pool = PgPoolOptions::new().max_connections(5).connect(&db_url).await.expect("Failed to connect to DB");
    ...
    tokio::spawn(async move {
        let aggregator = live_aggregator::Live1mAggregator::new(index_engine_clone, pool_clone, candle_tx_agg);
    ...
    tokio::spawn(async move {
        ...
        for (res, depth) in resolutions {
            ...
            let mut data_map: HashMap<i64, HashMap<String, (f64,f64,f64,f64,f64)>> = HashMap::new();
            ...
            for &asset in index_engine::Asset::all() {
                ...
                if let Ok(klines) = client.fetch_klines(sym, res, start_ts, now).await {
                    ...
                    for c in &klines {
                        ...
                        data_map.entry(ts).or_default()
                            .insert(short.clone(), (to_f(c.open), to_f(c.high), to_f(c.low), to_f(c.close), mcap));
            ...
            for ts_val in sorted_ts {
                ...
                if !index_assets.iter().all(|a| prices.contains_key(*a)) { continue; }

                let sqrt_caps: Vec<f64> = index_assets.iter().map(|a| prices[*a].4.sqrt()).collect();
                let total_sqrt: f64 = sqrt_caps.iter().sum();
                
                let (o, h, l, c) = {
                    let calc = |idx: usize| index_assets.iter().zip(&sqrt_caps).map(|(a, w)| {
                        let val = match idx { 0 => prices[*a].0, 1 => prices[*a].1, 2 => prices[*a].2, 3 => prices[*a].3, _ => 0.0 };
                        val * (w / total_sqrt)
                    }).sum::<f64>();
        ...
        loop {
            ...
            for &asset in index_engine::Asset::all() {
                if let Ok(klines) = client.fetch_klines(asset.binance_symbol(), "1m", Utc::now().timestamp() - 60, Utc::now().timestamp()).await {
                    if let Some(c) = klines.last() {
                        let res = index_engine_poller.update_price(index_engine::AssetPrice {
                            asset, price_usd: c.close, market_cap_usd: c.market_cap, timestamp: c.timestamp
                        }, None).await;
                        ...
                        for &r in &resolutions {
                            ...
                            let entry = live_candles.entry(key).or_insert((price_f, price_f, price_f, price_f, ts));
                            if entry.4 == ts {
                        ...
                        ticker_data.insert(short.clone(), json!({ "price": c.close, "change24h": change_24h }));
                        ...
                        if let Ok(Some(iv)) = res {
                            ...
                            for &r in &resolutions {
                                ...
                                let entry = live_candles.entry(key).or_insert((kxi_f, kxi_f, kxi_f, kxi_f, ts));
                                if entry.4 == ts {
                            ...
                            ticker_data.insert("KXI".to_string(), json!({ "price": iv.price, "change24h": kxi_change }));
                        }
                    }
                }
            }
