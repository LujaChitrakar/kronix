...
#[get("/chart/index")]
pub async fn get_index_chart(pool: web::Data<PgPool>, query: web::Query<ChartQuery>) -> impl Responder {
    let res = sqlx::query(
    ...
    match res {
        Ok(rows) => {
            let candles: Vec<serde_json::Value> = rows.into_iter().map(|r| {
                use sqlx::Row;
...
#[get("/chart/asset")]
pub async fn get_asset_chart(pool: web::Data<PgPool>, query: web::Query<ChartQuery>) -> impl Responder {
