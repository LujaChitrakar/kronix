use crate::states::{
    ...
    fixed_price_data, fixed_price_lots, oracle_pegged_price_offset,
};
...
// pegged orders with offset that let the price escape 1..i64::MAX range,
// this fn returns skipped adnd clamps price to that range
//
...
pub fn oracle_pegged_price(
    ...
    let price_offset = oracle_pegged_price_offset(price_data).unwrap();
...
fn key_for_fixed_price(key: u128, price_lots: i64) -> u128 {
    // oracle pegged price will always be >= 1
    assert!(price_lots >= 1);
...
// compares the fixed and oracle pegged price data and returns the one that would match first
// or worse if return_worse is set
pub fn rank_orders<'a>(
    ...
    match (fixed, oracle_pegged) {
        (Some(f), Some(o)) => {
            ...
            if is_better(
                f.1.key,
                key_for_fixed_price(u128::from_le_bytes(o.1.key), o.2).to_le_bytes(),
            ) ^ return_worse
            {
