use pinocchio::{error::ProgramError, sysvars::{Sysvar, clock::Clock}};

use crate::{errors::OrderBookError, states::{BookSideOrderTree, Orderbook, PostOrderType, Side, fixed_price_data}};
    ...
    /// Compute (price_lots, price_data) for this order
        ...
        pub fn price(
            ...
            let price_lots = match self.params {
                OrderParams::Market => {
                    market_order_limit_for_side(self.side)
                }
                OrderParams::ImmediateOrCancel { price_lots } => {
                    price_lots
                }
                OrderParams::FillOrKill { price_lots } => {
                    price_lots
                }
                OrderParams::Fixed { price_lots, order_type } => {
