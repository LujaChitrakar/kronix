...
const ASSETS: Record<
  AssetSymbol,
  { symbol: string; name: string; icon: string; accent: string; glow: string }
> = {
  BTC: {
...
export default function CryptoChart() {
  ...
  const switchSymbol = useCallback((asset: AssetSymbol) => {
    ...
    iframe.contentWindow.postMessage(
      {
        name: "set-symbol",
        data: { symbol: ASSETS[asset].symbol, interval: "D" },
      },
      "*",
    );
  }, []);
                ...
                {(Object.keys(ASSETS) as AssetSymbol[]).map((sym) => {
                  ...
                  return (
                    <button
                      ...
                      style={
                        { "--item-color": a.accent } as React.CSSProperties
                      }
