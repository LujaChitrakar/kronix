...
function pdaFingerprint(pda, hashVisitor) {
  return visitors.visit(nodes.pdaNode({ ...pda, name: "" }), hashVisitor);
}
function getUniquePdaName(name, usedNames) {
  if (!usedNames.has(name)) return name;
  let suffix = 2;
  let candidate = nodes.camelCase(`${name}${suffix}`);
  while (usedNames.has(candidate)) {
    suffix++;
    candidate = nodes.camelCase(`${name}${suffix}`);
  }
  return candidate;
}
function extractPdasVisitor() {
  return visitors.bottomUpTransformerVisitor([
    {
      select: "[programNode]",
      transform: (node) => {
...
function extractPdasFromProgram(program) {
  const hashVisitor = visitors.getUniqueHashStringVisitor();
  ...
  const rewrittenInstructions = program.instructions.map((instruction) => {
    const rewrittenAccounts = instruction.accounts.map((account) => {
      if (!account.defaultValue || !nodes.isNode(account.defaultValue, "pdaValueNode") || !nodes.isNode(account.defaultValue.pda, "pdaNode")) {
        return account;
      }
      ...
      if (!pdaMap.has(fingerprint)) {
        ...
        if (existingFingerprint !== void 0 && existingFingerprint !== fingerprint) {
          resolvedName = nodes.camelCase(`${instruction.name}_${pda.name}`);
          errors.logWarn(
        ...
        pdaMap.set(fingerprint, nodes.pdaNode({ ...pda, name: resolvedName }));
      }
      const extractedPda = pdaMap.get(fingerprint);
      const defaultValue = { ...account.defaultValue, pda: nodes.pdaLinkNode(extractedPda.name) };
      return nodes.instructionAccountNode({ ...account, defaultValue });
    });
...
function rootNodeFromAnchorV00(program, additionalPrograms = []) {
  ...
  return nodes.rootNode(programNode4, additionalProgramNodes);
}
...
exports.extractPdasFromProgram = extractPdasFromProgram;
exports.extractPdasVisitor = extractPdasVisitor;
