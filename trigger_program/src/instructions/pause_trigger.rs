...
pub fn process_pause_trigger(accounts: &[AccountView], _data: &[u8]) -> ProgramResult {
...
pub fn process_resume_trigger(accounts: &[AccountView], _data: &[u8]) -> ProgramResult {
    let [signer, trigger_order, _remaining @ ..] = accounts else {
        return Err(ProgramError::NotEnoughAccountKeys);
    };
    verify_signer(signer)?;
    ...
    if !order.is_paused() {
        return Err(TriggerProgramError::TriggerNotPaused.into());
    }
